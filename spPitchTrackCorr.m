...
% UPDATES:
% 1. Changed Line 60 - 61, for reducing the time vector by 1 point.
function [F0, T, R] = spPitchTrackCorr(x, fs, frame_length, frame_overlap, maxlag, show)
 ...
 T = (round(nsample/2):(nsample-noverlap):N-2-round(nsample/2))/fs;
...
if show 
    ...
    t = (0:N-1)/fs;
    plot(t, x);
    title('Song: 095. Spice Girls - Viva Forever__ex__black__foreground');
    legend('Waveform');
    xlabel('Time (s)');
    ylabel('Amplitude');
    xlim([t(1) t(end)]);
